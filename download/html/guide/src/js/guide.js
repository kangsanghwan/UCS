$(document).ready(function(){	
	...
	//Skip Navigation
	$('.skip-navigation').find('a').each(function(index, val){		
		$(this)
		.bind('focus', function(){
			$(this).css({'left':'0px'});
		})
	...
	$('.preview .open').click(function(){
		$(this).parents('.preview').addClass('active');
	});
    ...
    // File Upload
    $(".custom-upload-file input[type=file]").on("change", function () {
        if (this.files && this.files[0]) {
            var reader = new FileReader();
            var filename = $(this)[0].files[0].name;
            reader.addEventListener("load", function (e) {
                $(this).siblings('.u-preview').find('.u-image-preview').attr('src', e.target.result);
                $(this).siblings('.u-preview').find('.u-image-base64').val(e.target.result);
            });
        ...
        $(this).siblings('.u-text').val(filename);
    });
